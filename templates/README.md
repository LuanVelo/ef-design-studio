...
- **`dist/`** — pacotes `.eftpl` finais (zip; gerar sempre com paths forward-slash — `tar -a -cf pacote.zip ...` com o **bsdtar do Windows** (`C:\Windows\System32\tar.exe`) e renomear para `.eftpl`, ou montar o zip com o JSZip do app via node. O `tar` do Git Bash é GNU e ignora o `-a`, gerando um tar disfarçado; `Compress-Archive` grava backslashes — os dois quebram o JSZip.).
...
| `social-editorial/` | `ef-social-editorial-01` | feed-square (1080×1080), variant `manchete` | Figma "Social Media template" (`F79TjyIx9DEb3WK9Gl19Fe`), node `13:5` | Empacotado em `dist/` (2026-08-02); **pendente**: aprovação visual do usuário, fonte Presti Display (usando Playfair Display), thumbnail vindo do Figma — regerar do render real |
